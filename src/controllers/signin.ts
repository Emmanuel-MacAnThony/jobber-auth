...
import { BadRequestError, IAuthDocument, isEmail } from '@Emmanuel-MacAnThony/jobber-shared';
...
export async function read(req: Request, res: Response): Promise<void> {
  ...
  const passwordsMatch: boolean = await compare(password, existingUser.password as string);
  if (!passwordsMatch) {
    throw new BadRequestError('Invalid credentials', 'SignIn read() method error');
  }
